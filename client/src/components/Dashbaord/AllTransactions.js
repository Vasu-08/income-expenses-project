...
const AllTransactions = ({accountID}) => {
  const {getAllTransactionsAction, transactions} = useContext(TransactionsContext);
  
  useEffect(() => {
    getAllTransactionsAction(accountID);
  }, [getAllTransactionsAction, accountID]);
      ...
      <div className='px-4 sm:px-6 lg:px-8'>
        <div className='sm:flex sm:items-center'>
          <div className='sm:flex-auto'>
            ...
            <p className='mt-2 text-sm text-gray-700'>All transactions including expenses and income for this account</p>
          </div>
                    ...
                    {transactions?.map(transaction => {
                      return (
                        <tr key={transaction?.email} className={transaction?.color}>
                          ...
                          <td className='whitespace-nowrap px-3 py-4 text-sm text-gray-500'>
                            <span className='inline-flex rounded-full bg-green-100 px-2 text-xs font-semibold leading-5 text-green-800'>₹ {transaction?.amount}</span>
                          </td>
